...
class PerlWrapper(object):
    ...
    def __init__(self,name):
        """
        Initialization (create a new perl object)

        Note: Because ref is not defined yet, we need to set it over __dict__
        instead of self.ref
        """
    ...
    def __getattr__(self, name):
        """
        Interception of all get calls
        
        Intercepts all get calls to attributes that are NOT in self.__dict__

        This may either be be an attribute of the Perl object or a function
        call on the object.
        """

        if debug: 
            print('__getattr__ trace: get %s' % name)
    ...
    def __setattr__(self, name, value):
        ...
        """
        if debug: 
            print('__setattr__ trace: set %s to %s' % (name, value))
        ...
        # If it is not a Python attribute, we set the value in the Perl object
        else:
            if debug:
...
class ddb_api(type):
    ...
    def __new__(meta, classname, supers, classdict):
        ...
        for attr in classdict['_attr_data']:
            if debug: 
                print('adding getters and settes for ' + attr)
            if debug: 
                print('adding getters and settes for ' + attr + 
            ...
            # Create special getter/setters for debug mode ... 
            if debug: 
            ...
            # Check for read access, if present create the getter
            ...
            # Check for write access, if present create the setter
